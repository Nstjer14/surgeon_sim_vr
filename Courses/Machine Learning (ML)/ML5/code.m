...
figure();
plot(trainingdata(:,1), trainingdata(:,2), '.b') % The figure without any classes

% Settings for the Gaussian: 
k = 3; % The amount of clusters we want to make
Sigma = {'diagonal','full'}; % What kind Gaussions we want
nSigma = numel(Sigma);
...
for i = 1:nSigma;
    for j = 1:nSC;
        ...
        plot(gmfit.mu(:,1),gmfit.mu(:,2),'kx','LineWidth',2,'MarkerSize',10) % Plotting the data
